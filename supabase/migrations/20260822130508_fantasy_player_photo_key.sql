...
begin
  ...
  insert into public.fantasy_gameweek_scores (
    season_id, mapping_id, gameweek_id, points, event_transfers,
    event_transfers_cost, points_on_bench, source_synced_at, updated_at
  )
  select item.season_id, item.mapping_id, item.gameweek_id, item.points,
    item.event_transfers, item.event_transfers_cost, item.points_on_bench,
    p_synced_at, p_synced_at
  from jsonb_to_recordset(p_scores) as item(
    season_id uuid, mapping_id uuid, gameweek_id uuid, points integer,
  ...
  set last_validation_status = item.last_validation_status,
      last_error_message = case when item.last_validation_status = 'valid' then null else item.last_error_message end,
  ...
  from jsonb_to_recordset(p_mapping_results) as item(
    mapping_id uuid, last_validation_status text, last_error_message text,
...
begin
  ...
  select item.id, item.season_id, item.fpl_league_id, item.official_name,
    item.status, p_synced_at, 'succeeded', null, item.archived_at, p_synced_at
  ...
  select item.season_id, item.gameweek_id, item.fpl_entry_id,
    item.fpl_team_name, item.fpl_manager_name, item.points, item.event_transfers,
    item.event_transfers_cost, item.points_on_bench, p_synced_at, p_synced_at
  from jsonb_to_recordset(p_scores) as item(
