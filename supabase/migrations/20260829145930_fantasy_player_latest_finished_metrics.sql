alter table public.fantasy_player_gameweek_stats
  add column if not exists latest_finished_gameweek_defensive_contribution numeric(7,3),
  add column if not exists latest_finished_gameweek_bps integer,
  add column if not exists latest_finished_gameweek_number integer;
...
begin
  ...
  insert into public.fantasy_player_gameweek_stats (
    ...
    latest_finished_gameweek_defensive_contribution, latest_finished_gameweek_bps,
    latest_finished_gameweek_number, is_global_captain, is_global_vice_captain,
  ...
  select item.season_id, item.gameweek_id, item.fpl_player_id, item.photo_key,
    ...
    item.latest_finished_gameweek_defensive_contribution, item.latest_finished_gameweek_bps,
    item.latest_finished_gameweek_number, item.is_global_captain,
    item.is_global_vice_captain, p_synced_at, p_synced_at
  from jsonb_to_recordset(p_players) as item(
    ...
    latest_finished_gameweek_points integer,
    latest_finished_gameweek_defensive_contribution numeric,
    latest_finished_gameweek_bps integer, latest_finished_gameweek_number integer,
    is_global_captain boolean, is_global_vice_captain boolean
  )
  ...
  set photo_key = excluded.photo_key,
      ...
      latest_finished_gameweek_defensive_contribution = excluded.latest_finished_gameweek_defensive_contribution,
      latest_finished_gameweek_bps = excluded.latest_finished_gameweek_bps,
      latest_finished_gameweek_number = excluded.latest_finished_gameweek_number,
