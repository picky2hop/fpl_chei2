...
function PredictionResultLabel({ result }: { result: ReturnType<typeof getPredictionResult> }) {
...
function FixturePredictionCard({ fixture, choice, editable, onChoose }: { fixture: Fixture; choice?: PredictionChoice; editable: boolean; onChoose: (choice: PredictionChoice) => void }) {
  const choices: { id: PredictionChoice; label: string; sublabel: string }[] = [{ id: "home", label: "เหย้า", sublabel: fixture.homeTeam.shortName }, { id: "draw", label: "เสมอ", sublabel: "X" }, { id: "away", label: "เยือน", sublabel: fixture.awayTeam.shortName }];
  const score = getFixtureScoreText(fixture);
  const result = getPredictionResult(fixture, choice);
  return <article className={`rounded-[24px] border bg-[#10253a] p-4 shadow-[0_18px_45px_rgba(0,0,0,0.12)] ${editable ? "border-white/10" : "border-white/5 opacity-80"}`}><div className="mb-4 flex items-center justify-between"><p className="text-[11px] font-bold text-white/55">{fixture.dateLabel}</p><StatusPill fixture={fixture} /></div><div className="mb-4 flex items-center justify-center gap-4"><div className="flex min-w-0 flex-1 flex-col items-center gap-2 text-center"><TeamLogo team={fixture.homeTeam} /><p className="truncate text-xs font-extrabold">{fixture.homeTeam.name}</p>{choice === "home" && <PredictionResultLabel result={result} />}</div><div className="text-center"><p className="text-xl font-black">{score ?? "VS"}</p>{score && <p className="text-[10px] font-bold text-white/40">{fixture.status === "finished" ? "FULL TIME" : "LIVE SCORE"}</p>}{choice === "draw" && <PredictionResultLabel result={result} />}</div><div className="flex min-w-0 flex-1 flex-col items-center gap-2 text-center"><TeamLogo team={fixture.awayTeam} /><p className="truncate text-xs font-extrabold">{fixture.awayTeam.name}</p>{choice === "away" && <PredictionResultLabel result={result} />}</div></div>{!editable && <p className="mb-3 rounded-xl bg-white/5 px-3 py-2 text-center text-[10px] font-bold text-white/45">คู่นี้ปิดการทายแล้ว</p>}<div className="grid grid-cols-3 gap-2">{choices.map((item) => <button key={item.id} type="button" disabled={!editable} aria-pressed={choice === item.id} onClick={() => onChoose(item.id)} className={`rounded-2xl border py-2.5 transition disabled:cursor-not-allowed disabled:opacity-45 ${choice === item.id ? `${choiceColors[item.id]} border-transparent shadow-lg` : "border-white/10 bg-white/5 text-white/55 hover:bg-white/10"}`}><span className="block text-xs font-black">{item.label}</span><span className="mt-0.5 block text-[10px] font-bold opacity-75">{item.sublabel}</span></button>)}</div></article>;
}
...
function Predictions({ fixtures, predictions, editableFixtureIds, onChoose, onConfirm, isSaving, saveError }: { fixtures: Fixture[]; predictions: PredictionMap; editableFixtureIds: string[]; onChoose: (fixtureId: string, choice: PredictionChoice) => void; onConfirm: () => void; isSaving: boolean; saveError: string }) {
  ...
  return <section className="space-y-4"><div className="flex items-end justify-between"><div><p className="text-[10px] font-black uppercase tracking-[0.22em] text-[#8ca6bd]">Make your call</p><h2 className="mt-1 text-2xl font-black tracking-tight">ทายผลเกมวีคนี้</h2></div><span className="rounded-full bg-[#d9ff58] px-3 py-2 text-[11px] font-black text-[#071525]">{selected}/{fixtures.length} คู่</span></div><div className="rounded-2xl border border-[#d9ff58]/25 bg-[#d9ff58]/10 px-4 py-3 text-xs leading-5 text-[#d8eb96]">เลือกผลของคู่ที่ยังไม่เริ่ม แล้วกดบันทึกด้านล่าง <span className="font-black text-[#d9ff58]">ผลถูกได้ 3 แต้มหลังบอลจบ</span></div>{hasLockedFixtures && <p role="status" className="rounded-xl border border-[#ffc857]/25 bg-[#ffc857]/10 px-3 py-2 text-xs font-bold leading-5 text-[#ffe0a0]">{editableFixtureIds.length === 0 ? "ไม่ได้ทายคู่แรก จึงไม่สามารถทายคู่ที่เหลือของ GW นี้ได้" : "มีคู่ที่เริ่มแข่งแล้ว ระบบจะเก็บคำทายเดิม และบันทึกเฉพาะคู่ที่ยังไม่เริ่ม"}</p>}{saveError && <p role="alert" className="rounded-xl border border-[#ff647c]/30 bg-[#ff647c]/10 px-3 py-2 text-xs font-bold text-[#ffb0bc]">{saveError}</p>}<div className="space-y-3">{fixtures.map((fixture) => <FixturePredictionCard key={fixture.id} fixture={fixture} editable={editableFixtureIds.includes(fixture.id)} choice={predictions[fixture.id]} onChoose={(choice) => onChoose(fixture.id, choice)} />)}</div><button type="button" disabled={!complete || editableFixtureIds.length === 0 || isSaving} onClick={onConfirm} className={`flex w-full items-center justify-center gap-2 rounded-2xl py-3.5 text-sm font-black transition ${complete && editableFixtureIds.length > 0 && !isSaving ? "bg-[#d9ff58] text-[#071525] shadow-[0_12px_24px_rgba(217,255,88,0.15)] hover:bg-[#e7ff8c]" : "cursor-not-allowed bg-white/10 text-white/35"}`}>{isSaving ? "กำลังบันทึก…" : complete ? "ยืนยันคำทายที่ยังไม่เริ่ม" : editableFixtureIds.length === 0 ? "หมดสิทธิ์ทาย GW นี้" : `เลือกอีก ${editableFixtureIds.length - editableFixtureIds.filter((fixtureId) => predictions[fixtureId]).length} คู่เพื่อยืนยัน`} <ArrowRight size={16} /></button></section>;
}
...
export default function PredictionApp({ currentUser, gameweeks, fixturesByGameweek, leaderboardByGameweek, initialPredictionsByGameweek, initialGameweek, initialPredictionGameweek, predictionBookByGameweek: livePredictionBookByGameweek, onConfirmPredictions }: PredictionAppProps) {
  ...
  const editableFixtureIds = getEditablePredictionIds(editableFixtures, new Date(), predictions);
