...
function Leaderboard({ entries, gameweek, fixtureIds, predictionBook, onSelect }: { entries: LeaderboardEntry[]; gameweek: number; fixtureIds: string[]; predictionBook: Record<number, Record<string, PredictionMap>>; onSelect: (entry: LeaderboardEntry) => void }) {
  ...
  const visibleEntries = mode === "gameweek" ? getCompleteLeaderboardEntries(entries, fixtureIds, predictionBook, gameweek) : entries;
  const share = async () => {
    setShareMessage("");
    try {
  ...
  const onShare = share;
  return <section className="space-y-4"><div className="flex items-end justify-between gap-3"><div><p className="text-[10px] font-black uppercase tracking-[0.22em] text-[#8ca6bd]">Leaderboard</p><h2 className="mt-1 text-2xl font-black tracking-tight">ตารางคะแนน</h2></div><div className="flex rounded-full bg-white/10 p-1 text-[11px] font-bold"><button type="button" onClick={() => onModeChange("gameweek")} className={`rounded-full px-3 py-1.5 transition ${mode === "gameweek" ? "bg-[#d9ff58] text-[#071525]" : "text-white/55"}`}>GW {gameweek}</button><button type="button" onClick={() => onModeChange("season")} className={`rounded-full px-3 py-1.5 transition ${mode === "season" ? "bg-[#d9ff58] text-[#071525]" : "text-white/55"}`}>ทั้งฤดูกาล</button></div></div><div className="overflow-hidden rounded-[24px] border border-white/10 bg-[#10253a] shadow-[0_18px_45px_rgba(0,0,0,0.18)]"><div className="flex items-center justify-between border-b border-white/10 bg-white/5 px-4 py-3 text-[10px] font-black uppercase tracking-[0.16em] text-white/45"><span>Rank · Player</span><span>Points</span></div><div className="divide-y divide-white/10">{visibleEntries.length ? visibleEntries.map((entry) => { const points = mode === "season" ? entry.seasonPoints : entry.gameweekPoints; return <button type="button" key={entry.id} onClick={() => onSelect(entry)} className="flex w-full items-center gap-3 px-4 py-3.5 text-left transition hover:bg-white/5 focus-visible:bg-white/10"><span className={`w-5 text-center text-sm font-black ${entry.rank === 1 ? "text-[#d9ff58]" : "text-white/45"}`}>{String(entry.rank).padStart(2, "0")}</span><Avatar user={entry} size={38} /><div className="min-w-0 flex-1"><p className="truncate text-sm font-extrabold">{entry.displayName}</p><div className="mt-1 flex items-center gap-1.5"><div className="flex gap-1">{entry.form.map((value, index) => <span key={`${entry.id}-${index}`} className={`size-1.5 rounded-full ${value >= 3 ? "bg-[#47d7a0]" : value === 0 ? "bg-white/20" : "bg-[#ffc857]"}`} />)}</div><span className={`text-[10px] font-bold ${entry.trend === "up" ? "text-[#47d7a0]" : entry.trend === "down" ? "text-[#ff8698]" : "text-white/40"}`}>{entry.trend === "up" ? "↑ ขึ้นมา" : entry.trend === "down" ? "↓ หล่นลง" : "— เท่าเดิม"}</span></div></div><div className="text-right"><p className="text-lg font-black">{points}</p><p className="text-[10px] font-bold text-white/40">คะแนน</p></div></button>; }) : <p className="px-4 py-8 text-center text-sm font-bold text-white/45">ยังไม่มีผู้เล่นที่ทายครบ GW นี้</p>}</div></div><button type="button" onClick={onShare} className="flex w-full items-center justify-center gap-2 rounded-2xl border border-white/10 bg-white/5 py-3 text-sm font-extrabold text-white transition hover:bg-white/10">แชร์ตารางคะแนนเข้า LINE <Share2 size={15} /></button>{shareMessage && <p role="status" className="text-center text-[11px] font-bold text-[#d9ff58]">{shareMessage}</p>}<p className="text-center text-[11px] text-white/40">กดที่ชื่อผู้เล่นเพื่อดูคำทายของเขาใน GW นี้</p></section>;
}
...
function FixturePredictionCard({ fixture, choice, onChoose }: { fixture: Fixture; choice?: PredictionChoice; onChoose: (choice: PredictionChoice) => void }) {
  const choices: { id: PredictionChoice; label: string; sublabel: string }[] = [{ id: "home", label: "เหย้า", sublabel: fixture.homeTeam.shortName }, { id: "draw", label: "เสมอ", sublabel: "X" }, { id: "away", label: "เยือน", sublabel: fixture.awayTeam.shortName }];
  const editable = canEditPrediction({ status: fixture.status, kickoffAt: fixture.kickoff }, new Date());
...
function Predictions({ fixtures, predictions, onChoose, onConfirm, isSaving, saveError }: { fixtures: Fixture[]; predictions: PredictionMap; onChoose: (fixtureId: string, choice: PredictionChoice) => void; onConfirm: () => void; isSaving: boolean; saveError: string }) {
