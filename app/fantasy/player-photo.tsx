...
export function PlayerPhoto({ playerName, photoUrl, className = "size-10" }: { playerName: string; photoUrl?: string; className?: string }) {
  ...
  if (failed || !photoUrl) return <span aria-label={`รูป ${playerName} โหลดไม่ได้`} className={`grid ${className} shrink-0 place-items-center rounded-full bg-[#29435d] text-lg font-black text-white/80`}>⚽</span>;
