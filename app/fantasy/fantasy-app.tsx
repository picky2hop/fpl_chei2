...
function SquadPlayerRow({ player, bench }: { player: FantasySquadPlayer; bench: boolean }) {
  ...
  return <div className={`flex items-center gap-3 rounded-2xl border px-3 py-2.5 ${presentation.className}`}><PlayerPhoto playerName={player.playerName} photoUrl={player.photoUrl} className="size-18" /><div className="min-w-0 flex-1"><p className="truncate text-xs font-black">{player.playerName}{player.isCaptain ? " (C)" : player.isViceCaptain ? " (VC)" : ""}</p><p className="truncate text-[10px] font-bold opacity-65">{player.clubName} · {presentation.label}</p></div><span className="text-xs font-black">{player.points ?? "—"}</span></div>;
}
