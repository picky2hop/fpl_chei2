...
import { useFantasyBrowse } from "./use-fantasy-browse";
...
import { isGoalkeeperExcluded, playerStatsPositionOptions, rankVisiblePlayerStats, type PlayerStatsCategory, type PlayerStatsPositionFilter } from "@/lib/fantasy/player-stats-view";
...
function LegacyPlayerStats({ data, onOpenTeamOfWeek }: { data: FantasyLeagueDashboardResponse; onOpenTeamOfWeek: () => void }) {
  ...
  const excludeGoalkeeper = isGoalkeeperExcluded(category);
  const availablePositionKeys = playerStatsPositionOptions(category);
  const availablePositions = positions.filter((item) => availablePositionKeys.includes(item.key));
  const effectivePosition = excludeGoalkeeper && position === "GK" ? "ALL" : position;
  const visible = rankVisiblePlayerStats(data.playerStats, category, effectivePosition);
  ...
  async function shareStats() {
    ...
    const usesLatestFinishedGameweek = category === "defensiveContribution" || category === "bps" || category === "latestGameweekPoints";
    const result = await shareFantasyPlayerStats(liffShareApi(), {
      gameweek: usesLatestFinishedGameweek ? data.latestFinishedGameweek ?? data.currentGameweek : data.currentGameweek,
      categoryLabel: selectedCategory?.label ?? category,
      categoryDescription: selectedCategory?.description,
      excludeGoalkeeper,
      positionLabel: positions.find((item) => item.key === effectivePosition)?.label ?? effectivePosition,
...
export default function FantasyApp({ profile }: { profile: UserProfile }) {
  ...
  const { data: loadedData, context, error: browseError, retry } = useFantasyBrowse(selectedLeagueId, selectedGameweek);
  const data = loadedData ?? context;
  ...
  function openSquad(entry: SelectedEntry) {
    ...
    void fetch(`/api/fantasy/team?${query}`).then(async (response) => {
      ...
      const highlightPlayerIds = loadedData?.playerOfWeek.state === "ready" ? loadedData.playerOfWeek.value.players.map((player) => player.playerId) : [];
  ...
  if (!data) return <main className="flex min-h-screen flex-col items-center justify-center gap-3 bg-[#071525] px-5 text-sm font-bold text-white/60">{browseError || error || "กำลังโหลดข้อมูล Fantasy…"}{browseError && <button type="button" onClick={() => void retry()} className="rounded-xl bg-[#d9ff58] px-4 py-2 text-[#071525]">ลองใหม่</button>}</main>;
  return <main className="min-h-screen bg-[#071525] px-5 py-7 text-white"><div className="mx-auto max-w-xl"><header className="flex items-center justify-between gap-3"><div><p className="text-[10px] font-black uppercase tracking-[0.24em] text-[#8ca6bd]">FPL CHEI CHEI</p><h1 className="mt-1 text-3xl font-black">แฟนตาซีเชยเชย</h1><p className="mt-1 text-xs font-bold text-white/45">{profile.displayName} · {data.season.name}</p></div><Avatar name={profile.displayName} avatarUrl={profile.avatarUrl} /></header>{browseError && <p role="alert" className="mt-3 text-xs text-[#ffc857]">{browseError} <button type="button" onClick={() => void retry()} className="underline">ลองใหม่</button></p>}<p className="mt-2 text-[10px] text-white/45">ซิงก์ล่าสุด: {data.sync.lastSyncedAt ? new Date(data.sync.lastSyncedAt).toLocaleString("th-TH", { timeZone: "Asia/Bangkok" }) : "ยังไม่มีข้อมูล"}</p>{data.sync.stale && <div className="mt-5 rounded-2xl border border-[#ffc857]/30 bg-[#ffc857]/10 px-4 py-3 text-xs font-bold leading-5 text-[#ffe0a0]">ข้อมูลอาจยังไม่ใช่รอบล่าสุด · {data.sync.message}</div>}<div className="mt-6 flex flex-wrap items-center justify-between gap-2"><div className="flex gap-2"><Link href="/" className="rounded-full border border-white/10 bg-white/5 px-3 py-2 text-xs font-black text-white/70">หน้าแรก</Link><Link href="/dashboard" className="rounded-full border border-white/10 bg-white/5 px-3 py-2 text-xs font-black text-white/70">ไปแอปทายผล</Link></div></div><label className="mt-4 block text-xs font-bold text-white/60">ลีกที่กำลังดู<select aria-label="เลือกลีก Fantasy" value={selectedLeagueId} onChange={(event) => { setSelectedLeagueId(event.target.value); setSelectedGameweek(null); }} className="mt-1 w-full rounded-xl border border-white/15 bg-[#10253a] px-3 py-3 text-sm font-black text-white">{leagues.leagues.map((league) => <option key={league.id} value={league.id}>{league.official_name} · {league.status === "active" ? "ใช้งาน" : "เก็บประวัติ"}</option>)}</select></label><div className="mt-4 flex flex-wrap items-center justify-between gap-2"><button type="button" onClick={() => setSelectedGameweek(null)} disabled={selectedGameweek === null} className="rounded-full border border-[#d9ff58]/30 bg-[#d9ff58]/10 px-3 py-2 text-[10px] font-black text-[#d9ff58] disabled:opacity-45">GW ปัจจุบัน: GW {data.currentGameweek}</button><select aria-label="เลือกเกมวีค" value={selectedGameweek ?? data.currentGameweek} onChange={(event) => setSelectedGameweek(Number(event.target.value))} className="rounded-full border border-white/15 bg-[#10253a] px-3 py-2 text-xs font-black text-white outline-none"><option value={data.currentGameweek}>GW {data.currentGameweek}</option>{Array.from({ length: data.currentGameweek - 1 }, (_, index) => index + 1).map((gameweek) => <option key={gameweek} value={gameweek}>GW {gameweek}</option>)}</select></div><div className="mt-6 flex rounded-2xl border border-white/10 bg-[#10253a] p-1"><button type="button" onClick={() => setTab("leaderboard")} className={`flex-1 rounded-xl py-3 text-sm font-black ${tab === "leaderboard" ? "bg-[#d9ff58] text-[#071525]" : "text-white/55"}`}>ตารางคะแนน</button><button type="button" onClick={() => setTab("players")} className={`flex-1 rounded-xl py-3 text-sm font-black ${tab === "players" ? "bg-[#d9ff58] text-[#071525]" : "text-white/55"}`}>สถิตินักเตะ</button></div><div className="mt-6">{loadedData ? (tab === "leaderboard" ? <Leaderboard data={loadedData} leagueName={loadedData.leagues.find((league) => league.id === selectedLeagueId)?.official_name ?? "Fantasy League"} gameweek={loadedData.selectedLeaderboardGameweek} onSelectEntry={openSquad} /> : <PlayerStats data={loadedData} />) : <section role="status" className="rounded-3xl border border-white/10 bg-[#10253a] p-8 text-center text-sm text-white/60">{browseError ? "ยังโหลดตารางที่เลือกไม่สำเร็จ" : "กำลังโหลดตารางที่เลือก…"}</section>}</div></div>{selectedEntry && <CurrentSquadModal entry={selectedEntry} loading={squadLoading} response={squadResponse} error={squadError} onShare={() => void shareSquad()} isSharing={isSquadSharing} shareStatus={squadShareStatus} onClose={() => { setSelectedEntry(null); setSquadResponse(null); setSquadError(""); setSquadShareStatus(null); }} />}</main>;
}
