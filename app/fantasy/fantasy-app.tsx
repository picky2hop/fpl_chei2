...
import { playerPresentation } from "@/lib/fantasy/player-presentation";
...
type SelectedEntry = { entryId: number; managerName: string; teamName: string };

function SquadPlayerRow({ player, bench }: { player: FantasySquadPlayer; bench: boolean }) {
  const presentation = playerPresentation(player.position, bench);
  return <div className={`flex items-center gap-3 rounded-2xl border px-3 py-2.5 ${presentation.className}`}><PlayerPhoto playerId={player.playerId} playerName={player.playerName} photoUrl={player.photoUrl} className="size-9" /><div className="min-w-0 flex-1"><p className="truncate text-xs font-black">{player.playerName}{player.isCaptain ? " (C)" : player.isViceCaptain ? " (VC)" : ""}</p><p className="truncate text-[10px] font-bold opacity-65">{player.clubName} · {presentation.label}</p></div><span className="text-xs font-black">{player.points ?? "—"}</span></div>;
}

function CurrentSquadModal({ entry, loading, response, error, onClose }: { entry: SelectedEntry; loading: boolean; response: CurrentSquadResponse | null; error: string; onClose: () => void }) {
  return <div className="fixed inset-0 z-50 grid place-items-end bg-[#020914]/80 p-3 sm:place-items-center" role="presentation" onClick={onClose}><section className="max-h-[90vh] w-full max-w-xl overflow-y-auto rounded-[28px] border border-white/15 bg-[#10253a] p-5 shadow-2xl" role="dialog" aria-modal="true" aria-labelledby="fantasy-current-squad-title" onClick={(event) => event.stopPropagation()}><div className="flex items-start justify-between gap-4"><div><p className="text-[10px] font-black uppercase tracking-[0.2em] text-[#8ca6bd]">Current FPL squad</p><h2 id="fantasy-current-squad-title" className="mt-1 text-xl font-black">ทีมของ {entry.managerName}</h2><p className="mt-1 text-xs font-bold text-white/45">{entry.teamName} · FPL {entry.entryId}</p></div><button type="button" onClick={onClose} aria-label="ปิดทีม Fantasy" className="grid size-9 shrink-0 place-items-center rounded-full border border-white/10 bg-white/5 text-xl text-white/70">×</button></div>{loading && <p className="mt-8 py-8 text-center text-sm font-bold text-white/55">กำลังโหลดทีม GW ปัจจุบัน…</p>}{!loading && error && <p className="mt-6 rounded-2xl border border-[#ff647c]/30 bg-[#ff647c]/10 px-4 py-4 text-sm font-bold leading-6 text-[#ff9aaa]">{error}</p>}{!loading && !error && response && <div className="mt-5 space-y-5"><div className="flex flex-wrap gap-2"><span className="rounded-full border border-[#d9ff58]/30 bg-[#d9ff58]/10 px-3 py-2 text-xs font-black text-[#d9ff58]">GW {response.squad.gameweekNumber}</span><span className="rounded-full border border-white/10 bg-white/5 px-3 py-2 text-xs font-black text-white/65">แผน {response.squad.formation}</span></div><div><h3 className="mb-2 text-xs font-black uppercase tracking-[0.18em] text-white/45">ตัวจริง · {response.squad.starters.length} คน</h3><div className="space-y-2">{response.squad.starters.map((player) => <SquadPlayerRow key={player.pickPosition} player={player} bench={false} />)}</div></div><div><h3 className="mb-2 text-xs font-black uppercase tracking-[0.18em] text-white/45">ตัวสำรอง · {response.squad.bench.length} คน</h3><div className="space-y-2">{response.squad.bench.map((player) => <SquadPlayerRow key={player.pickPosition} player={player} bench />)}</div></div><p className="text-[10px] font-bold leading-5 text-white/35">แสดงเฉพาะทีมของ GW ปัจจุบัน และระบบจะเขียนทับ snapshot เดิมเมื่อขึ้น GW ใหม่</p></div>}</section></div>;
}

function Leaderboard({ data, gameweek, onSelectEntry }: { data: FantasyLeagueDashboardResponse; gameweek: number; onSelectEntry: (entry: SelectedEntry) => void }) {
...
function PlayerStats({ data }: { data: FantasyLeagueDashboardResponse }) {
  ...
  return <section className="space-y-4"><div><p className="text-[10px] font-black uppercase tracking-[0.22em] text-[#8ca6bd]">Global FPL player stats</p><h2 className="mt-1 text-2xl font-black">สถิตินักเตะ GW {data.currentGameweek}</h2></div><div className="rounded-2xl border border-[#6da9ff]/20 bg-[#6da9ff]/10 px-4 py-3 text-xs leading-5 text-[#b8d4ff]">ข้อมูลนักเตะรวมจาก FPL ไม่เปลี่ยนตามลีกที่เลือก และเก็บ snapshot นักเตะทั้งหมด</div>{groups.map((group) => <div key={group.label} className="rounded-[24px] border border-white/10 bg-[#10253a] p-4"><h3 className="text-sm font-black">{group.label}</h3><div className="mt-3 grid gap-3 sm:grid-cols-2">{positions.map((position) => <div key={position} className="rounded-2xl bg-white/5 p-3"><p className={`inline-flex rounded-full border px-2 py-1 text-[10px] font-black uppercase tracking-[0.15em] ${playerPresentation(position, false).className}`}>{position}</p><div className="mt-2 space-y-2">{group.values[position].slice(0, 6).map((player, index) => <div key={player.playerId} className="flex items-center gap-2"><span className="w-4 text-center text-[10px] font-black text-white/40">{index + 1}</span><PlayerPhoto playerId={player.playerId} playerName={player.playerName} className="size-8" /><div className="min-w-0 flex-1"><p className="truncate text-xs font-bold">{player.playerName}</p><p className="truncate text-[9px] font-bold text-white/40">{player.clubName}</p></div><span className="text-xs font-black text-[#d9ff58]">{player.metricValue}</span></div>)}{group.values[position].length === 0 && <p className="text-[10px] text-white/35">ยังไม่มีข้อมูล</p>}</div></div>)}</div></div>)}</section>;
}
