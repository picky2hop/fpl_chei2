...
test("Fantasy API returns the requested historical gameweek for authenticated users", async () => {
  ...
  const response = await handler(new Request("https://example.test/api/fantasy?gameweek=1"));
  assert.equal(response.status, 200);
  assert.deepEqual(requested, { userId: "user-1", gameweekNumber: 1 });
});
...
test("Fantasy API rejects invalid gameweek before loading data", async () => {
  ...
  const response = await handler(new Request("https://example.test/api/fantasy?gameweek=39"));
...
test("Fantasy API keeps auth and database errors generic", async () => {
  ...
  assert.equal((await unauthenticated(new Request("https://example.test/api/fantasy"))).status, 401);

  const failed = createFantasyHandler({
    requireUser: async () => ({ id: "user-1" }),
    getDashboard: async () => { throw new Error("secret database detail"); },
  });
  const response = await failed(new Request("https://example.test/api/fantasy"));
